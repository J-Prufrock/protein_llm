...
SYSTEM_PROMPT = "You are a helpful antibody analysis assistant."
ANTIBODY_PREFIX_TOKEN = "<ANTIBODY_PREFIX>"
ANTIBODY_CONTEXT_LABEL = "Antibody context:"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.set_defaults(use_pretoken=True)
...
def split_indices(
    ...
    remaining_after_validation = num_samples - validation_size
    if test_split > 0 and remaining_after_validation > 1:
        test_size = max(1, test_size)
    if validation_size + test_size >= num_samples:
        test_size = max(0, num_samples - validation_size - 1)

    validation_indices = indices[:validation_size]
    test_start = validation_size
    test_end = validation_size + test_size
    test_indices = indices[test_start:test_end]
    train_indices = indices[test_end:]
    if not train_indices:
        raise ValueError("Training split is empty after applying validation_split and test_split.")
...
def build_user_content(instruction: str, use_pretoken: bool) -> str:
    if not use_pretoken:
        return instruction
    return f"{ANTIBODY_CONTEXT_LABEL}\n{ANTIBODY_PREFIX_TOKEN}\n\n{QUESTION_LABEL}\n{instruction}"


def ensure_antibody_prefix_token(tokenizer: AutoTokenizer) -> int:
    additional_special_tokens = list(getattr(tokenizer, "additional_special_tokens", []) or [])
    if ANTIBODY_PREFIX_TOKEN not in additional_special_tokens:
        tokenizer.add_special_tokens({"additional_special_tokens": additional_special_tokens + [ANTIBODY_PREFIX_TOKEN]})

    antibody_prefix_token_id = tokenizer.convert_tokens_to_ids(ANTIBODY_PREFIX_TOKEN)
    if antibody_prefix_token_id is None or antibody_prefix_token_id < 0:
        raise ValueError(f"Failed to register special token {ANTIBODY_PREFIX_TOKEN} in tokenizer.")
    return int(antibody_prefix_token_id)
...
def filter_overlength_samples(
    ...
    for index, sample in enumerate(samples):
        prompt_ids = build_prompt_token_ids(tokenizer, str(sample["instruction"]), use_template, use_pretoken)
        placeholder_count = prompt_ids.count(antibody_prefix_token_id) if use_pretoken else 0
        if use_pretoken and placeholder_count != 1:
            dropped_count += 1
            continue
        ...
        if use_pretoken:
            total_length = total_length - 1 + prefix_length
...
def build_dataloaders(
    ...
    if test_indices:
        test_dataset = PreparedAFDDataset(*subset_cache(samples, antibody_embeddings, test_indices))
    ...
    if is_main_process():
        ...
        print(
            f"Train samples: {len(train_indices)}, validation samples: {len(validation_indices)}, "
            f"test samples: {len(test_indices)}"
        )
    return train_loader, validation_loader, test_loader, train_sampler, validation_sampler, test_sampler
...
def build_prompt_token_ids(tokenizer: AutoTokenizer, instruction: str, use_template: bool, use_pretoken: bool) -> list[int]:
    user_content = build_user_content(instruction, use_pretoken)
    if use_template:
        try:
            return list(
    ...
    return list(tokenizer(fallback_prompt, add_special_tokens=False)["input_ids"])
...
def build_model_inputs(
    ...
    for sample_index, (instruction, answer) in enumerate(zip(batch["instructions"], batch["answers"])):
        ...
        if use_pretoken and placeholder_count != 1:
            ...
            raise ValueError(f"Sample {sample_id} must contain exactly one {ANTIBODY_PREFIX_TOKEN} placeholder.")
        placeholder_position = prompt_ids.index(antibody_prefix_token_id) if use_pretoken else -1
        ...
        if use_pretoken:
            prompt_embeddings = torch.cat(
                [
                    ...
                    prefix_embeddings[sample_index],
                    prompt_token_embeddings[placeholder_position + 1 :],
                ],
                dim=0,
            )
        else:
            prompt_embeddings = prompt_token_embeddings

        answer_tensor = torch.tensor(answer_ids + [eos_token_id], dtype=torch.long, device=device)
        answer_embeddings = embedder(answer_tensor).to(dtype=embedding_dtype)

        # Replace the placeholder token inside the prompt template with the full antibody prefix embedding.
        sample_input_embeddings = torch.cat([prompt_embeddings, answer_embeddings], dim=0)
        sample_attention_mask = torch.ones(sample_input_embeddings.size(0), dtype=torch.long, device=device)

        prompt_labels = torch.full((prompt_embeddings.size(0),), -100, dtype=torch.long, device=device)
        answer_labels = torch.tensor(answer_ids + [eos_token_id], dtype=torch.long, device=device)
        sample_labels = torch.cat([prompt_labels, answer_labels], dim=0)
...
def train() -> None:
    ...
    antibody_prefix_token_id = ensure_antibody_prefix_token(tokenizer) if config.use_pretoken else -1
    ...
    if is_main_process():
        ...
        print(f"use_pretoken={config.use_pretoken}")

    train_loader, validation_loader, test_loader, train_sampler, validation_sampler, test_sampler = build_dataloaders(
        samples,
        antibody_embeddings,
        config,
        tokenizer,
    ...
    if is_main_process() and test_loss is not None:
        test_summary = {
            "timestamp": datetime.now().isoformat(timespec="seconds"),
            "test_loss": test_loss,
        }
        print(json.dumps(test_summary, ensure_ascii=False, indent=2))
        append_epoch_log(log_path, test_summary)
    cleanup_distributed()
